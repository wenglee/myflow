...
class DFlow():
	...
	def run(cls, args=args):
		...
		if step is not None:
			...
			if phase in cls._dag:
				result = None
				try:
					result = dask.async.get_sync(cls._dag,phase)
		...
		for method in DFlow._args.keys():
			...
			while i < len(DFlow._args[method]):
				if (_is_odd(i)):
					...
					#	parser.set_defaults(arg = BFlow._args[method][i]['default'])
					else:
			...
			if sys.version_info > (3,1):
				...
				if (isinstance(result, types.GeneratorType)):
					...
					if result['step'] == step:
						_log_dataframe(result)
						return(result[return_type])
		raise ValueError("no result for %s" % step)
